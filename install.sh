...
check_token() {
  ...
  if [ "$USER_TOKEN" = "vinzzoffc" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di VinzzOfficial${NC}"
    echo -e "${YELLOW}TELEGRAM : @VinzzOfficial${NC}"
    echo -e "${YELLOW}WHATSAPP : +6283832320022${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© VinzzOfficial${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  ...
  THEME_URL="https://github.com/VinzzOfficial/Pterodactyl-Theme-Autoinstaller/raw/main/${THEME_NAME}.zip"
...
# Unduh file tema
wget -O /root/stellar.zip https://github.com/VinzzOfficial/Pterodactyl-Theme-Autoinstaller/raw/main/stellar.zip
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer VinzzOfficial   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : VinzzOfficial      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : VinzzOfficial  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by VinzzOfficial  ${NC}"
